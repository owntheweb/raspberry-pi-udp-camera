...
raspivid -n -t 0 -w $WIDTH -h $HEIGHT -g $INTRA_REFRESH_PERIOD $FLIP_HORIZONTAL $FLIP_VERTICAL -rot $ROTATION -fps $FRAMES_PER_SECOND -b $BITRATE -mm $METERING_MODE -ex $EXPOSURE_MODE -ih -pf high -awb greyworld -o udp://$STREAM_DESTINATION_IP:$STREAM_DESTINATION_PORT
